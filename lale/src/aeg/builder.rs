...
impl AEGBuilder {
    ...
    pub fn build(&self, initial_state: MicroArchState, max_cycles: u32) -> Result<AEG, String> {
        ...
        while let Some((current_state, current_node, cycles)) = worklist.pop_front() {
            ...
            if aeg.node_count() >= self.max_states {
                return Err(format!("State space explosion: exceeded {} states", self.max_states));
            }
            ...
            for successor in successors {
                ...
                if self.enable_joining {
                    if let Some(existing_node) = self.find_joinable_state(&aeg, &successor) {
                        ...
                        let joined = self.simulator.join(
                            &aeg.graph[existing_node].state,
                            &successor
                        );
    ...
    fn find_joinable_state(&self, aeg: &AEG, state: &MicroArchState) -> Option<petgraph::graph::NodeIndex> {
...
mod tests {
    use super::*;
    use crate::microarch::state::{PlatformConfig, CacheConfig, CacheLevelConfig, MemoryConfig, MemoryLatency, ReplacementPolicy};
