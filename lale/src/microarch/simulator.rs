...
use super::pipeline::{InstructionSlot, Opcode, StageType, AbstractAddress, MemoryAccessType};
...
impl MicroArchSimulator {
    ...
    fn split_on_cache_uncertainty(&self, state: &MicroArchState, address: u64) -> Vec<MicroArchState> {
...
mod tests {
    use super::*;
    use crate::microarch::state::{CacheConfig, CacheLevelConfig, MemoryConfig, MemoryLatency, ReplacementPolicy};
    use crate::microarch::pipeline::{RegisterDep, DepType, MemoryAccess};
    ...
    fn test_pipeline_advancement() {
        ...
        assert!(state.pipeline.get_stage(StageType::Fetch).unwrap().instruction.is_some());
        
        // Advance pipeline
        simulator.advance_pipeline(&mut state);
        
        // Instruction should have moved to decode
        assert!(state.pipeline.get_stage(StageType::Fetch).unwrap().instruction.is_none());
        assert!(state.pipeline.get_stage(StageType::Decode).unwrap().instruction.is_some());
    }
    ...
    fn test_fetch_instruction() {
        ...
        let instr = InstructionSlot {
            ...
            opcode: Opcode::ALU,
            dependencies: vec![
                RegisterDep { register: 1, dep_type: DepType::Read },
                RegisterDep { register: 2, dep_type: DepType::Write },
            ],
            memory_access: None,
        };
