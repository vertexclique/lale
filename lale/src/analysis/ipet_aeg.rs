...
impl AEGIPETSolver {
    ...
    pub fn solve_wcet(
        ...
        for node in aeg.graph.node_indices() {
            ...
            let incoming: Expression = aeg.graph
                .edges_directed(node, Direction::Incoming)
                .map(|e| edge_vars[&e.id()])
                .sum();
            
            let outgoing: Expression = aeg.graph
        ...
        for loop_info in loops {
            ...
            if let Some(&header_node) = aeg.block_map.get(&format!("{:?}", header_label)) {
                // Entry edges to loop
                let entry_edges: Expression = aeg.graph
                ...
                // Backedges in loop
                let backedges: Expression = aeg.graph
                    .edges_directed(header_node, Direction::Incoming)
                    .filter(|e| {
                        let src = e.source();
...
mod tests {
    use super::*;
    use crate::aeg::compression::{CompressedAEG, BlockNode, BlockEdge};
